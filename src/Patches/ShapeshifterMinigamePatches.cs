...
{
    private const int HandlingId = 30017;
    ...
    public static bool Prefix(ShapeshifterMinigame __instance)
    {
        try
        {
            ...
            for (int i = 0; i < playerList.Count; i++)
            {
                ...
                shapeshifterPanel.SetPlayer(i, playerData, (Il2CppSystem.Action) (() =>
                {
                    ...
                    catch (Exception ex) { ErrorReporter.Report(ex, HandlingId, "ShapeshifterMinigame_Begin.Prefix: pick player target"); }
                }));
        ...
        catch (Exception ex) { ErrorReporter.Report(ex, HandlingId, "ShapeshifterMinigame_Begin.Prefix: build pick menu"); return true; }
    }
}
...
public static class ShapeshifterPanel_SetPlayer
{
    ...
    public static bool Prefix(ShapeshifterPanel __instance, int index, NetworkedPlayerInfo playerInfo, Il2CppSystem.Action onShift)
    {
        ...
        catch (Exception ex) { ErrorReporter.Report(ex, HandlingId, "ShapeshifterPanel_SetPlayer.Prefix: set pick menu player"); return true; }
    }
}
