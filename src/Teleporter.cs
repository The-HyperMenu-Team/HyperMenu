...
namespace MalumMenu
{
    internal class Teleporter
    {
        ...
        public static void TeleportToVent(PlayerControl player, int ventId)
        {
            ...
            {
                MalumMenu.notifications.Send("Vent TP", "The game must have started in order for this feature to work");
                return;
            }
            ...
            if(!features.Troll.VentSeqIds.ContainsKey(player))
            {
                features.Troll.VentSeqIds.Add(player, 10000);
            }

            MessageWriter enterVent = MessageWriter.Get(SendOption.None);
            enterVent.Write(++features.Troll.VentSeqIds[player]);
            ...
            bootFromVent.Write(++features.Troll.VentSeqIds[player]);
